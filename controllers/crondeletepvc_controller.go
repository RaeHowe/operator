...
func (r *CronDeletePVCReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	//cronDeletePVC的cr里面，status.DeletedPVCs集合里面包含了要删除的pvc信息，如果在map里面包含了的话，就把集合里面的pvc元素删除掉
	for _, item := range cronDeletePVC.Status.DeletedPVCs {
		if _, ok := pvcMap[item.Name]; !ok {
